...
export const PlinkoGame: React.FC<PlinkoGameProps> = ({ game, currency, balance, user, onClose, onSpin, isPaused, onVisualBalanceChange }) => {
  ...
  const dropBall = async () => {
      ...
      try {
          ...
          if (outcome) {
              ...
              const newBall: Ball = { id: ballIdCounter.current++, path: outcome.path, currentStep: 0, progress: 0, finished: false, rotation: Math.random() * 360, winAmount: winAmount, multiplier: outcome.multiplier };
  ...
  const renderBuckets = () => multipliers.map((mult, i) => {
      ...
      // Dynamic Sizing to prevent overlap
      // For high row counts (16), width must be extremely small to fit on mobile
      let widthClass = 'w-8 h-8 text-[9px]'; // Default (8-12 rows)
      if (rowCount > 14) widthClass = 'w-3 h-5 text-[5px]'; // 15-16 rows (Extreme compaction)
      else if (rowCount > 12) widthClass = 'w-5 h-6 text-[7px]'; // 13-14 rows
