...
export const Card: React.FC<CardProps> = ({ rank, suit, hidden, className = "", style, theme = 'classic' }) => {
  ...
  const witchPalette = {
      primary: isRed ? '#db2777' : '#8b5cf6', // Magenta vs Violet
      secondary: isRed ? '#831843' : '#5b21b6',
      dark: '#1c1917', // Stone 900
      surface: '#292524', // Stone 800
      accent: '#10b981', // Emerald (Magic)
      border: '#4c1d95',
      font: 'serif' // Mystical font fallback
  };

  const palette = isWitch ? witchPalette : classicPalette;
      ...
      if (['J', 'Q', 'K'].includes(rank)) {
          return (
            <g transform="translate(50, 70)">
                <rect x="-25" y="-35" width="50" height="70" rx="5" fill="none" stroke={color} strokeWidth="1" opacity="0.4" />
                <path d="M-15 -15 L15 -15" stroke={accent} strokeWidth="2" />
                <path d="M0 -30 L0 30" stroke={accent} strokeWidth="1" />
                <circle cx="0" cy="0" r="20" fill="none" stroke={color} strokeWidth="2" />
                <text x="0" y="5" fontSize="50" fill={color} opacity="0.2" fontWeight="900" textAnchor="middle" dominantBaseline="middle" fontFamily={palette.font}>{rank}</text>
            </g>
          );
      }
      return null;
  ...
  return (
    ...
    >
      ...
      >
        <defs>
            {/* --- SUIT GRADIENTS (Theme Aware) --- */}
            <linearGradient id={gradRedId} x1="0" y1="0" x2="1" y2="1">
                <stop offset="0%" stopColor={isWitch ? '#f472b6' : '#fb7185'} />
                <stop offset="100%" stopColor={isWitch ? '#be185d' : '#e11d48'} />
            </linearGradient>
            <linearGradient id={gradBlackId} x1="0" y1="0" x2="1" y2="1">
                <stop offset="0%" stopColor={isWitch ? '#a78bfa' : '#38bdf8'} />
                <stop offset="100%" stopColor={isWitch ? '#6d28d9' : '#0284c7'} />
            </linearGradient>
            <linearGradient id={gradGoldId} x1="0" y1="0" x2="1" y2="1">
                <stop offset="0%" stopColor={isWitch ? '#34d399' : '#fcd34d'} />
                <stop offset="100%" stopColor={isWitch ? '#059669' : '#d97706'} />
            </linearGradient>
            ...
            <pattern id={cardBackPatternId} x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse">
                 <rect width="20" height="20" fill={palette.dark} />
                 {isWitch ? (
                     <path d="M10 0 L20 10 L10 20 L0 10 Z" fill="none" stroke="#4c1d95" strokeWidth="1"/>
                 ) : (
                     <path d="M0 0 L20 20 M20 0 L0 20" stroke="#334155" strokeWidth="1" />
                 )}
            </pattern>
        ...
        {hidden ? (
          // --- CARD BACK ---
          <>
            <rect x="5" y="5" width="90" height="130" rx="5" fill={`url(#${cardBackPatternId})`} stroke={palette.border} strokeWidth="2"/>
            {isWitch ? (
                <g transform="translate(50,70)">
                    <circle r="25" fill="none" stroke="#8b5cf6" strokeWidth="2" />
                    <path d="M0 -20 L18 12 L-18 12 Z" fill="none" stroke="#8b5cf6" strokeWidth="2" transform="rotate(180)"/>
                    <path d="M0 -20 L18 12 L-18 12 Z" fill="none" stroke="#8b5cf6" strokeWidth="2" />
                </g>
            ) : (
                <>
                    <circle cx="50" cy="70" r="20" fill={palette.dark} stroke={palette.accent} strokeWidth="2" />
                    <text x="50" y="78" fontSize="24" fontFamily="Arial" fontWeight="bold" fill={palette.accent} textAnchor="middle">GG</text>
                </>
            )}
          ...
          <>
            <g transform="translate(12, 22)">
                <text x="0" y="0" fontSize="18" fontWeight="800" fill={`url(#${isRed ? gradRedId : gradBlackId})`} textAnchor="middle" style={{ fontFamily: palette.font }}>{rank}</text>
