...
export const Layout: React.FC<LayoutProps> = ({ 
  ...
  const NavItem = ({ view, label, icon }: { view: ViewType, label: string, icon: React.ReactNode }) => {
    ...
    // On mobile, the menu is always full width when open, so we always show the label
    ...
    return (
        <button 
            ...
            `}
        >
            <span className={`${showLabel && 'mr-3'} ${isActive ? 'text-white' : 'group-hover:text-white'}`}>
                {icon}
            </span>
            {showLabel && <span className="whitespace-nowrap opacity-100 transition-opacity duration-200">{label}</span>}
        </button>
    );
  };
  ...
  return (
    <div className="flex h-screen bg-[#020617] text-slate-100 overflow-hidden font-sans">
        ...
        `}>
            {/* Sidebar Header */}
            <div className={`h-16 lg:h-20 flex items-center border-b border-slate-800 transition-all duration-300 ${!isMobile && sidebarCollapsed ? 'justify-center' : 'justify-between px-6'}`}>
            ...
            {/* Show expand button if collapsed (Desktop Only) */}
            {!isMobile && sidebarCollapsed && (
            ...
            {/* Mobile Wallet Section (Visible only on Mobile Drawer) */}
            {isMobile && user && (
                <div className="p-4 mx-3 mt-3 bg-slate-800/50 rounded-xl border border-slate-700/50 animate-in fade-in slide-in-from-left-4 duration-300">
                    <div className="flex justify-between items-start mb-3">
                         <div>
                            <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider mb-1">Current Balance</p>
                            <div className="flex items-baseline gap-1">
                                 ...
                                 <span className="text-xs font-bold text-slate-500">{currency === CurrencyType.GC ? 'GC' : 'SC'}</span>
                            </div>
                         </div>
                         <CurrencyToggle className="scale-90 origin-top-right" />
                    </div>
                    <button onClick={() => { setView('get-coins'); setMobileMenuOpen(false); }} className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 rounded-lg text-xs font-bold text-white shadow-lg shadow-indigo-900/20 transition-all flex items-center justify-center gap-2">
            ...
            <nav className="flex-1 p-3 overflow-y-auto hide-scrollbar space-y-4">
                ...
                <div className={`pt-4 border-t border-slate-800 ${!isMobile && sidebarCollapsed ? 'flex flex-col items-center' : ''}`}>
                    {(isMobile || !sidebarCollapsed) && <p className="px-4 text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Cashier</p>}
            ...
            <div className={`p-4 border-t border-slate-800 ${!isMobile && sidebarCollapsed ? 'flex justify-center' : ''}`}>
                {user ? (
        ...
        <div className="flex-1 flex flex-col min-w-0 relative">
            ...
            <header className="h-16 lg:h-20 bg-slate-900/90 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-4 lg:px-8 z-40 sticky top-0 shadow-lg">
                <div className="flex items-center gap-2">
                    ...
                    {/* Home Button - Visible on Mobile/Tablet or always if desired */}
                    <button 
                ...
                {user && (
                    <div className="flex items-center gap-2 sm:gap-4 animate-in fade-in slide-in-from-top-2 duration-300">
                        ...
                        {/* Mobile Balance Pill (Header Version - Simplified) */}
                         <div className="sm:hidden flex items-center bg-slate-950 rounded-full px-3 py-1 border border-slate-700">
                        ...
                        <button onClick={() => setView('get-coins')} className="w-10 h-10 rounded-full bg-gradient-to-r from-indigo-600 to-purple-600 flex items-center justify-center text-white font-bold shadow-lg shadow-indigo-500/30 hover:shadow-indigo-500/50 hover:scale-110 transition-all scale-90 sm:scale-100">
